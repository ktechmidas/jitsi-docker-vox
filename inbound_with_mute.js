...
const INBOUND_CALL_DURATION_LIMIT = 5 * 60 * 1000;
let callAlerting;
let selectedRoom;
let confId;
let confJID;
...
function joinRoom(room) {
    ...
    const outboundCall = VoxEngine.callUser({
        ...
        extraHeaders : { 'X-Room-Name'    : encodeURI(room),
                         'VI-CallTimeout' : 5, "X-Domain-Base": true,
                       },
        video        : false,
    ...
    outboundCall.addEventListener(CallEvents.InfoReceived, e => handleReceivedInfo(e));
    outboundCall.addEventListener(CallEvents.Connected,
    () => {
        timeoutHandler(inboundCall, INBOUND_CALL_DURATION_LIMIT);
        startIVR(inboundCall, outboundCall);
    });
}
...
VoxEngine.addEventListener(AppEvents.CallAlerting, (e) => {
    ...
    inboundCall.answer({}, { audioLevelExtension: true })
});
